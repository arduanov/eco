...
class MainController extends FrontEndController
{
	...
	protected function send_mime_mail($name_from, $email_from, $name_to, $email_to, $data_charset, $send_charset, $subject, $body, $html = false, $filepath = [], $filename = [])
	{
	...
	protected function sending($subject, $labels, $filepath = [], $filename = [], $message = '')
	{
		if (empty($message)) {
			$i = 0;
			foreach ($labels as $k => $v) {
				if ($v == '') {
					if (!in_array($k, ['Комментарий'])) {
		...
		if (empty($this->emails)) {
			$this->emails = Settings::model()->getMail();
		}
	...
	protected function pagination($href, $count, &$page, $lim)
	{
		...
		if (count($request_uri) > 1) {
			$href_sufix .= '?' . $request_uri[1];
		}
		/*выполнение программы*/
		$maxpage = round($count / $lim);
		if ($maxpage < $count / $lim) {
			$maxpage++;
		}
		if ($page > $maxpage) {
			$page = $maxpage;
...
//				if ($page > 1) {
//					$pagination_menu_prev = '<a href="' . $href . ($page - 1) . $href_sufix . '" class="prev">Пред.</a>';
//				}
...
//					// прижимаем вправо
//					if (2 * $point_num == $page - $near - $begend - 1) {
//						$point_num--;
//					}
//					$point_num = $page - $near - $point_num - 1;
//					// приклеиваем к левой границе
//					if ($point_num + $near + 1 < $page - $near) {
//						$point_num = $page - 2 * $near - 1;
//					}
//					// вывод в переменную
...
//					// прижимаем влево
//					if (2 * $point_num == $page - $near - ($maxpage - $begend) - 1) {
//						$point_num--;
//					}
...
//			if ($get_year == $year[$page - 1]->year) {
				$pagination_menu .= '<li><a class="active" href="' . $href . '">Все</a></li>';
//			}

			for ($i = 1; $i < $year_count + 1; $i++) {
...
//					$pagination_menu .= '<li><a href="' . $href . '?y=' . $year[$i - 1]->year . '">' . $year[$i - 1]->year . '</a></li>';
//				} else {
//					$pagination_menu .= '<li><span>.vvv..</span></li>';
//				}
//				/**/
//				for ($i = $page - $near; $i < $page; $i++) {
//					$i_page = $year_page[$i - 1];
...
//			} else {
//				$pagination_menu .= '<li class="active"><span>Всvvvе</span></li>';
...
//					$pagination_menu .= '<li><a href="' . $href . '?y=' . $year[$i - 1]->year . '">' . $year[$i - 1]->year . '</a></li>';
//				} else {
//					$pagination_menu .= '<li><span>.nnn..</span></li>';
//				}
//				/**/
//				for ($i = $maxpage - $begend + 1; $i < $maxpage + 1; $i++) {
//					$i_page = $year_page[$i - 1];
...
//			}
	...
	protected function year_pagination_by_page($href, $count, &$page, $lim, $year)
	{
		...
		$year_page = [];
		for ($i = 0; $i < $year_count; $i++) {
			if ($count_before_page + $lim > $count_before_page_i) {
				$page = $i + 1;
			}
	...
	protected function get_id($url = '', $tree = [], $level = 0, $type = '')
	{
		$prev_id = 0;
		$id = 0;
		$next_id = 0;
		...
		for ($i = 0; $i < $count; $i++) {
			foreach ($tree as $k => $t) {
				...
				if ($t['code'] == $url[$i]) {
					...
					$tree_next = [];
					if (is_array($t['child']) && count($t['child']) > 0) {
						$tree_next = $t['child'];
					}
					$next_id = -1;
				} else {
...
//        die;
		...
		$hidden_pages = [];
		$pages_id_records_templates = [
			'lenta_files_category' => [], //файлы с категориями
			'lenta_records_simple' => [], //простой список
			'lenta_records' => [], //для вакансий?
			'magazine' => [], //для магазина
			'consult' => [] //для вопрос/ответ
		];
		...
		if (empty(Yii::app()->request->pathInfo)/*  || empty(Yii::app()->user->id) */) {
			...
			$data['news_common_list'] = ModuleNews::model()->getList(ModulesInPages::model()
			                                                                       ->getLink($data['news_common_doc_id'], 'news'), 0, 5, 1);
					...
					default:
						switch ($data['doc_id']) {
							...
							default:
								...
								if (/*in_array($data['doc_id'],$pages_id_redirect_to_next_level)*/
								!in_array($data['doc_id'], $pages_id_not_redirect_to_next_level)
								) {
									$temp_array = [];
	...
	protected function moduleComplaint_book($data)
	{
		...
		if (substr($type, 0, 5) == 'page_' || empty($type)) { // выбор страницы
			...
			$data['url_for_menu'] = substr(substr($data['pages']->make_url($data['doc_id_last']), 0, -1), 1);
			$data['include'] = ['forms/complaint_book'];
	...
	protected function moduleCatalog_decision($data)
	{
		...
		// фильтры
		$params = [];
		if (isset($_GET['wear_id'])) {
			$params['wear_id'] = $_GET['wear_id'];
		}
		...
		if (isset($_GET['size_id']) && (int)$_GET['size_id']) {
			$params['size_id'] = (int)$_GET['size_id'];
		}
		$data['params'] = $params;
		...
		if (substr($type, 0, 5) == 'page_' || empty($type)) { // выбор страницы
			...
			$data['item'] = $decision->getList(['ids' => [$item_id]], 0, 0);
			if (count($data['item']) == 0) {
				$this->redirect($pages->make_url($data['doc_id_last']));
			} else {
