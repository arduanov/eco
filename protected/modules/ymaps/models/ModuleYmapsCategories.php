...
/**
 * This is the model class for table "rktv_module_ymaps_categories".
 *
 * The followings are the available columns in table 'rktv_module_ymaps_categories':
 * @property integer $id
 ...
 * @property ModuleYmaps[] $moduleYmaps
 * @property ModulesInPages $mpage
 */
class ModuleYmapsCategories extends CActiveRecord{

	public $points = array();
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return ModuleYmapsCategories the static model class
	 */
	...
	public function tableName(){
		return 'rktv_module_ymaps_categories';
	}
	...
	public function relations(){
		...
		return array(
			'moduleYmaps' => array(self::HAS_MANY, 'ModuleYmaps', 'category_id'),
	 ...
	 * @return type 
	 */
	public function getList($mpage_id, $offset = 0, $limit = 0, $active = NULL, $points = NULL){
		$data = array();
		$criteria = new CDbCriteria();
		if(!is_null($active)){
		...
		foreach($this->findAll($criteria) as $value){
			...
			if(!is_null($points) && $points){
				$data[$value->id]->points = ModuleYmaps::model()->getList($value->id, $active);
			}
		}
		return $data;
	}
