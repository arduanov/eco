			...
			<ul class="<?=($role_id<2 && count($list)>1)?'sortable':''?> type_1">
				<? foreach($list as $item_id => $item): ?>
					<li data-id="<?= $item->id ?>" class="link ids">
						...
						<? elseif($role_id<2 && in_array($item->data_type_id,array(9))): ?>
							<a href="/admin.php?r=/<?= $this->module->id ?>/params/list4&page_id=<?= $page_id ?>&id=<?= $item->id ?>" class="alert_title"><?= $item->title ?></a>&nbsp;&nbsp;&nbsp;
						<? elseif($role_id<2): ?>
						...
						<? if($role_id<2): ?>
							...
							<br><i style="color: #666;"><?=$item->type_title?> [code: <?=$item->code?>]</i>
						<? endif; ?>
					</li>
				<?php endforeach; ?>
			</ul>
