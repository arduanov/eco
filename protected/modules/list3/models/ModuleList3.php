...
/**
 * This is the model class for table "rktv_module_list3".
 *
 * The followings are the available columns in table 'rktv_module_list3':
 * @property integer $id
 ...
 */
class ModuleList3 extends CActiveRecord
{
	public $img = array();
	 ...
	 * @return ModuleList3 the static model class
	 */
	...
	public function getList($mpage_id, $offset = 0, $limit = 0, $active = NULL){
        ...
        foreach($this->findAll($criteria) as $value){
            ...
            $data[$value->id]->params = ModuleList3Values::model()->getListWithParams($value->id);
        }
        return $data;
	}
    ...
    public function getImageById($id){
        $result = null;
        if(!empty($id))
            $result = '/'.Files::model()->getUploadFolder('list3', false, true).Files::model()->findByPk($id)->file_name;
        else
            $result = null;
