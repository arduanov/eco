...
class DefaultController extends BackEndController
{
	...
	public function actionIndex()
	{
           ...
           if(Yii::app()->request->isAjaxRequest){
               ...
               /*  ������ ������  */
               $_POST = array(
                   "table"=>"rktv_module_catalog_product",
                   "delimiter"=>";",
                   "mode"=>4,
                   "perRequest"=>1,
                   "csvKey"=>"3",
                   "tableKey"=>"article",
                   "columns"=>array(
                       "id"=>"", "category_id"=>"", "name"=>1, "short"=>"", "text"=>"", "small_img_id"=>"", "large_img_id"=>"", "price"=>2, "article"=>3, "unit"=>"", "recomendate"=>"",
                   ),
               );
               $_POST['thirdStep'] = 1;
        ...
        public function actionUpload()
        {
            ...
            $uploaddir  = Yii::app()->controller->module->path;
            $uploadfile = $uploaddir.'catalog_price.csv';
	...
	public function actionUpdatePrices()
	{
        ...
        try {
            ...
            $upload_folder = $_SERVER['DOCUMENT_ROOT'].'/csv/';
            ...
            if(isset($result["error"]) && strlen($result["error"]) > 0){
				...
				$delimiter    = ',';
				// $uploadfile   = 'csv/prices.csv';
				$uploadfile   = $upload_folder.$result['filename'];
				$filecontent  = explode("\n",file_get_contents($uploadfile));
				unlink($uploadfile); //Удаление файла
				foreach($filecontent as $f){
					$csv = explode($delimiter,$f);
					if(count($csv)==2){
						$articul .= " WHEN articul='$csv[0]' THEN '$csv[1]'";
						$code .= " WHEN code='$csv[0]' THEN '$csv[1]'";
					
					};
				};
				$command = $connection->createCommand("UPDATE rktv_module_catalog_suite SET price= CASE $code ELSE price END");
				$sql_result = $command->execute();
				$message .= '<p><b>Комплекты</b><br>Обновлено: '.$sql_result.' шт.</p>';
				$command = $connection->createCommand("UPDATE rktv_module_catalog_price SET price= CASE $articul ELSE price END");
				$sql_result = $command->execute();
				$message .= '<p><b>Модули</b><br>Обновлено: '.$sql_result.' шт.</p>';
				$command = $connection->createCommand("UPDATE rktv_module_catalog_accessories SET price= CASE $articul ELSE price END");
				$sql_result = $command->execute();
				$message .= '<p><b>Аксессуары</b><br>Обновлено: '.$sql_result.' шт.</p>';
