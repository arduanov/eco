...
class Module_gmaps extends CWidget {
    
    public $page_id = null;
    public $page = 1;
    public $module_id = 'gmaps';
    ...
    public function run() {
        ...
        $model = new ModuleGmapsCategories();
        if (!is_null($page_id) && Pages::model()->existsPage($page_id)) {
            $this->create_item($page_id, $model);
            $this->render('module_gmaps', array('model' => $model, 'page_id' => $page_id));
        } else {
    ...
    public function create_item($page_id = null, $model) {
        ...
        if (isset($_POST['ModuleGmapsCategories'])) {
            $_POST['ModuleGmapsCategories']['mpage_id'] = ModulesInPages::model()->getLink($page_id, $this->module_id);
